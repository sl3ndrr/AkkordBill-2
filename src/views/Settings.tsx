      ...
      <div className="settings-layout">
        ...
        <div className="settings-content">
          ...
          <section id="history" className="surface settings-section">
            ...
            {state.voidedInvoiceNumbers.length > 0 && <div className="number-register"><div><h3>Reservierte Nummern gelöschter Rechnungen</h3><p>Diese Nummern bleiben dauerhaft belegt und werden nicht erneut vergeben.</p></div>{state.voidedInvoiceNumbers.map((entry) => <div className="number-register__row" key={`${entry.number}-${entry.deletedAt}`}><span><strong>{entry.number}</strong><small>{entry.recipient} · {entry.amount.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}</small></span><time dateTime={entry.deletedAt}>gelöscht {new Intl.DateTimeFormat('de-DE', { day: '2-digit', month: '2-digit', year: 'numeric' }).format(new Date(entry.deletedAt))}</time></div>)}</div>}
          </section>
